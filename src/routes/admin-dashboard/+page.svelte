      ...
      {#each $userEntries as entry (entry.email)}
        <tr>
          ...
          <td>{entry.last_name}</td>
					...
					<td>{entry.last_sign_in_at === undefined ? "N/A" : new Date(entry.last_sign_in_at).toLocaleString()}</td>
          <td class="database-view-link text-center">
            <TextButton callback={() => deleteUser(entry.id, entry.first_name + " " + entry.last_name)} style="text-danger">
              <Trash />
            </TextButton>
          </td>
        </tr>
